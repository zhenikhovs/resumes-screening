...
VACANCIES_DIR = "data/raw/vacancies"
...
def fetch_vacancies(token, queries, per_query=100):
    """
    Скачивает вакансии по списку query.
    На каждый query — отдельный файл.

    queries: list[str]
    per_query: int (сколько вакансий на один query)
    """

    logger = setup_logger("data/prepared/fetch_vacancies.log")
    ...
    for query in queries:
        ...
        path = f"{VACANCIES_DIR}/vacancies_{query.replace(' ', '_')}.json"
