"""Ранжирование эмбеддингами RuSBERT. Score per-vacancy min-max → [0, 1]."""
import numpy as np
from tqdm import tqdm
from sentence_transformers import SentenceTransformer, util

from config.paths import RESUMES_TRANSFORMER, VACANCIES_TRANSFORMER, RUSBERT_RESULTS
...
RUSBERT_RESULTS.mkdir(parents=True, exist_ok=True)
...
def run_rusbert():
    for vacancy_file in sorted(VACANCIES_TRANSFORMER.glob("vacancies_*.json")):
        ...
        results = []
        for v in tqdm(vacancies, desc=f"RuSBERT [{query}]"):
        ...
        save_json(RUSBERT_RESULTS / f"rusbert_{query}.json", results)
        print(f"[+] RuSBERT {query} → {RUSBERT_RESULTS / f'rusbert_{query}.json'}")
