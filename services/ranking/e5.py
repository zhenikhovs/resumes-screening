"""Ранжирование эмбеддингами E5. Score per-vacancy min-max → [0, 1]."""
from pathlib import Path
import numpy as np
from tqdm import tqdm
from sentence_transformers import SentenceTransformer, util

from config.paths import RESUMES_TRANSFORMER, VACANCIES_TRANSFORMER, E5_RESULTS
...
E5_RESULTS.mkdir(parents=True, exist_ok=True)
...
def run_e5():
    for vacancy_file in sorted(VACANCIES_TRANSFORMER.glob("vacancies_*.json")):
        ...
        results = []
        for v in tqdm(vacancies, desc=f"E5 [{query}]"):
        ...
        save_json(E5_RESULTS / f"e5_{query}.json", results)
        print(f"[+] E5 {query} → {E5_RESULTS / f'e5_{query}.json'}")
