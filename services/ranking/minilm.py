"""Ранжирование эмбеддингами MiniLM. Score per-vacancy min-max → [0, 1]."""
import numpy as np
from tqdm import tqdm
from sentence_transformers import SentenceTransformer, util

from config.paths import RESUMES_TRANSFORMER, VACANCIES_TRANSFORMER, MINILM_RESULTS
...
MINILM_RESULTS.mkdir(parents=True, exist_ok=True)
...
def run_minilm():
    for vacancy_file in sorted(VACANCIES_TRANSFORMER.glob("vacancies_*.json")):
        ...
        results = []
        for v in tqdm(vacancies, desc=f"MiniLM [{query}]"):
        ...
        save_json(MINILM_RESULTS / f"minilm_{query}.json", results)
        print(f"[+] MiniLM {query} → {MINILM_RESULTS / f'minilm_{query}.json'}")
