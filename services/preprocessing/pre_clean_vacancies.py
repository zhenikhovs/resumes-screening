"""Удаление лишних полей и очистка HTML из сырых полных вакансий → pre-cleaned.

Вход: data/raw/full/vacancies/ (только что полученные из HH полные вакансии).
Выход: data/prepared/vacancies/pre-cleaned/
"""
from pathlib import Path
from bs4 import BeautifulSoup

from config.paths import RAW_FULL_VACANCIES, PRE_CLEANED_VACANCIES
from services.utils import load_json, save_json

PRE_CLEANED_VACANCIES.mkdir(parents=True, exist_ok=True)

FIELDS_TO_REMOVE = [
    "premium", "billing_type", "can_upgrade_billing_type", "relations", "insider_interview",
    "response_letter_required", "department", "show_contacts", "contacts",
    "branded_description", "vacancy_constructor_template", "auto_response",
    "accept_handicapped", "accept_kids", "age_restriction", "archived", "response_url",
    "adv_response_url", "code", "hidden", "quick_responses_allowed", "driver_license_types",
    "accept_incomplete_resumes", "negotiations_url", "suitable_resumes_url", "apply_alternate_url",
    "alternate_url", "has_test", "test", "show_logo_in_search", "closed_for_applicants",
    "fly_in_fly_out_duration", "internship", "night_shifts", "type", "published_at",
    "created_at", "initial_created_at", "allow_messages", "employer", "area",
]
...
def clean_vacancy(vacancy: dict) -> dict:
    ...
    if "address" in vacancy and isinstance(vacancy["address"], dict):
        cleaned["address"] = {"city": vacancy["address"].get("city")}
    ...
    if "employer" in vacancy and isinstance(vacancy["employer"], dict):
        cleaned["employer"] = {"id": vacancy["employer"].get("id"), "name": vacancy["employer"].get("name")}
    return cleaned


def process_files():
    for file_path in sorted(RAW_FULL_VACANCIES.glob("vacancies_*.json")):
        data = load_json(file_path)
        if not isinstance(data, list):
            print(f"⚠ Файл {file_path.name} не содержит список вакансий, пропускаем")
            continue
        cleaned_data = [clean_vacancy(v) for v in data]
        save_path = PRE_CLEANED_VACANCIES / file_path.name
